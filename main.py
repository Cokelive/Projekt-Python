...
def Frame1():
    ...
    def Prowadz():
        ...
        Frame2.geometry('850x300+500+300')
...
#            bar.start()
            ...
            sql2_result = db_connect_and_fetch(sql2, var2)
            url = sql2_result[0][0]
            btnMail['state']='disabled'
            
#            threading.Thread(target=worker_start, args=(fdir, url,)).start()
            now = time.time()
            datetime = time.strftime("_%m.%d.%Y_%H%M%S")
            filename = 'strona_' + combo.get() + datetime + '.pdf'
            
            pdfkit.from_url(url, filename)
            recv = txt.get('1.0', 'end-1c')
            with open('mail.lista', 'w') as fl:
                fl.writelines(recv)
...
#            print(recv)
            ...
            if recv != '':
                toaddrs  = recv   
                pdffiles = [filename]
                gs.sendOnEmail(toaddrs, pdffiles)
            
            
            btnMail['state']='enabled'
            
        def send():
            barMail.grid(column=1,row=3)
            barMail.grid()
            barMail.start()
#            threading.Thread(target=sendEmail).start()
        ...
        lblWpiszEmail = Label(Frame2, text = "Dopisz swoj e-mail do listy:")
        lblWpiszEmail.grid(column=0,row=2)
        
        txt = scrolledtext.ScrolledText(Frame2, width=40, height=10)
        with open('mail.lista') as fl:
            mailList = fl.readlines()
        mailList = "".join(mailList)
        txt.insert(INSERT, mailList)
#        txt = Entry(Frame2, width=30)
        txt.grid(column=0, row=3)
        barMail = Progressbar(Frame2, orient=HORIZONTAL, length=100, mode='indeterminate')
        btnMail = Button(Frame2, text="Wyslij na e-mail\n(docelowo automatycznie)", command=send)
        btnMail.grid(column=1, row=3)
